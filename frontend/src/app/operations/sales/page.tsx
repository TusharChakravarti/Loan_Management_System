        ...
        <main className="max-w-7xl mx-auto p-6 space-y-6">
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
            <div className="p-6 border-b border-slate-100">
              <h2 className="text-lg font-black text-slate-900">Sales Application Queue</h2>
              <p className="text-slate-500 text-xs mt-0.5">
                Verify applicant salary slips, BRE results, and forward applications to Sanction Desk.
              </p>
            </div>

            {loading ? (
              <div className="p-12 text-center text-slate-400 text-xs font-bold animate-pulse">
                Loading Sales review applications...
              </div>
            ) : error ? (
              <div className="p-6 bg-rose-50 text-rose-700 text-xs font-bold">{error}</div>
            ) : loans.length === 0 ? (
              <div className="p-12 text-center text-slate-400 text-xs font-semibold">
                No pending loan applications awaiting Sales review.
              </div>
            ) : (
            ...
            <div className="bg-white rounded-2xl shadow-xl border border-slate-200 w-full max-w-lg p-6 space-y-5">
              <div className="flex justify-between items-center border-b border-slate-100 pb-3">
                <h3 className="text-base font-black text-slate-900">
                  Review Application #{selectedLoan._id.slice(-6).toUpperCase()}
                </h3>
              ...
              <div className="bg-slate-50 p-4 rounded-xl space-y-2 text-xs">
                ...
                <p><strong className="text-slate-500 font-semibold">Monthly Salary:</strong> ₹{selectedLoan.monthlySalary.toLocaleString('en-IN')}</p>
                <p><strong className="text-slate-500 font-semibold">Loan Amount:</strong> ₹{selectedLoan.loanAmount.toLocaleString('en-IN')} for {selectedLoan.tenureDays} Days</p>
              </div>
              ...
              <div>
                ...
                <textarea
                  rows={3}
                  placeholder="e.g. Salary slip verified, salary credit matches bank statement."
                  value={remarks}
              ...
              <div className="flex justify-end gap-3 pt-2 border-t border-slate-100">
                ...
                >
                  {submitting ? 'Processing...' : 'Approve & Forward to Sanction →'}
                </button>
              </div>
            </div>
          </div>
